...
import {
  ModelCarousel,
  type CarouselModel,
} from '@/components/workspace/ModelCarousel';
...
const textCarouselModels: CarouselModel[] = [
  {
    name: 'ChatGPT',
    desc: 'Универсальный ИИ от OpenAI',
    gradient: 'linear-gradient(135deg, #10a37f, #1a7f5a)',
    badge: 'TOP',
  },
  {
    name: 'Claude',
    desc: 'Глубокий анализ от Anthropic',
    gradient: 'linear-gradient(135deg, #d4a27f, #8b5e3c)',
    badge: 'TOP',
  },
  {
    name: 'Gemini',
    desc: 'Мультимодальный ИИ от Google',
  ...
  {
    name: 'Grok',
    desc: 'ИИ от xAI с поиском',
    gradient: 'linear-gradient(135deg, #1d1d1f, #3a3a3c)',
  },
  {
    name: 'Perplexity',
    desc: 'Поиск с источниками',
    gradient: 'linear-gradient(135deg, #20b2aa, #008080)',
  },
  {
    name: 'Qwen',
    desc: 'Мощный китайский ИИ',
...
function useColors() {
  ...
  return {
    ...
    inputShadowFocus:
      '0 0 0 3px rgba(232, 84, 32, 0.12), 0 1px 4px rgba(0,0,0,0.2)',
    cardBg: 'var(--bg-card)',
...
const TextPage = () => {
  ...
  const provider =
    textProviders.find((p) => p.id === providerId) || textProviders[0];
  const subModel =
    provider.subModels.find((s) => s.id === subModelId) ||
    provider.subModels[0];
  ...
  return (
    <ErrorBoundary>
      <div
        className='flex flex-col h-[calc(100vh-var(--header-height,64px))] mesh-background'
      ...
      >
        ...
        <div className='flex-1 overflow-y-auto relative z-[1]'>
          ...
          {!hasMessages && (
            <div
              className='px-3 sm:px-4 lg:px-8 py-6 space-y-6 border-t mt-8 sm:mt-16'
              style={{ borderColor: c.divider }}
            >
              <ModelCarousel
                models={textCarouselModels}
                onSelect={(name) => {
                  const p = textProviders.find(
                    (pr) => pr.name.toLowerCase() === name.toLowerCase(),
                  );
        ...
        {/* ─── Input area ─── */}
        <div
          className='shrink-0 px-3 sm:px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-2 relative z-[1]'
          style={{ background: c.bg }}
        >
          <div className='max-w-[780px] mx-auto'>
            ...
            >
              ...
              >
                ...
                <div
                  className='flex items-center gap-2 mt-2 pt-2 overflow-x-auto no-scrollbar'
                ...
                >
                  ...
                  <button
                    ...
                    style={
                      input.trim() && !isGenerating
                        ? {
                            background: c.sendActive,
                            boxShadow: c.sendActiveShadow,
                          }
                        : {
                            background: c.sendInactive,
                            color: c.sendInactiveText,
                          }
                    }
                  >
    ...
    <div className='max-w-[780px] mx-auto py-6 px-4 space-y-5'>
      {messages.map((msg) => (
        ...
        >
          {msg.role === 'assistant' && (
            <ModelIcon
              providerId={msg.providerId || currentProviderId}
              size={24}
              className='mt-1'
            />
          )}
      ...
      <div className='max-w-[780px] mx-auto space-y-3'>
        ...
        <div className='flex items-center gap-2 flex-wrap'>
          ...
          <div className='ml-auto flex gap-2'>
            ...
            <button
              onClick={onClose}
              className='px-4 py-1.5 rounded-full text-xs text-white font-medium'
              style={{ background: c.sendActive }}
            >
              Применить
            </button>
          </div>
