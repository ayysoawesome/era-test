...
} from '@/components/workspace/WelcomeBlock';
import {
  MediaChatFeed,
  type MediaGeneration,
} from '@/components/workspace/MediaChatFeed';
import { ErrorBoundary } from '@/components/text/shared/ErrorBoundary';
...
const welcomeScenarios: WelcomeScenario[] = [
  {
    ...
    prompt:
      'Product showcase commercial, smooth camera movement, clean white studio, premium lighting, slow motion details',
    providerId: 'kling',
  ...
  },
  {
    ...
    prompt:
      'Music video scene, cinematic color grading, dynamic transitions, artistic visual effects, concert atmosphere',
    providerId: 'kling',
    subModelId: 'kling-3.0',
    aspect: '16:9',
    duration: '10s',
  ...
  },
  {
    ...
    prompt:
      'Character animation, smooth movement, expressive gestures, 3D rendered style, professional quality',
    providerId: 'kling',
    subModelId: 'kling-3.0',
    aspect: '1:1',
    duration: '5s',
  ...
  },
  {
    ...
    prompt:
      'Cinematic establishing shot, golden hour lighting, anamorphic lens flare, shallow depth of field, film grain',
    providerId: 'veo',
    aspect: '21:9',
    duration: '5s',
    resolution: '1080p',
  },
  {
    ...
    prompt:
      'Trendy vertical video, fast cuts, text overlay ready, vibrant colors, social media optimized, engaging hook',
    providerId: 'kling',
    subModelId: 'kling-3.0',
    aspect: '9:16',
    duration: '5s',
  ...
  },
  {
    ...
    prompt:
      'Animated greeting card, floating particles, warm colors, celebration mood, gentle camera movement, sparkle effects',
    providerId: 'kling',
...
const VideoPage = () => {
  ...
  const videoScenarios = [
    {
      ...
      gradient: 'linear-gradient(135deg, #1a0533, #2d1250)',
    },
    {
    ...
    {
      name: 'Анимация персонажа',
      gradient: 'linear-gradient(135deg, #1a0a2e, #2a1a3e)',
    },
    {
      name: 'Кинематографичная сцена',
      gradient: 'linear-gradient(135deg, #0a1628, #162040)',
    },
    {
      name: 'Короткий клип для Reels',
      gradient: 'linear-gradient(135deg, #1a1030, #2a1840)',
    },
    {
      name: 'Видеопоздравление',
      gradient: 'linear-gradient(135deg, #0d2018, #1a3028)',
    },
    {
      name: 'Продуктовое видео',
      gradient: 'linear-gradient(135deg, #1a1520, #2a2030)',
    },
  ];
  ...
  const videoGridModels = [
    {
      name: 'Kling 3.0',
      desc: 'Топовое видео от Kling AI',
      isNew: true,
      credits: 75,
    },
    {
      name: 'Seedance 2.0',
      desc: 'Плавные многокадровые видео',
      isNew: true,
      badges: ['60% OFF'],
      credits: 48,
    },
    ...
    {
      name: 'Wan AI',
      desc: 'Точное управление видео',
      isNew: true,
      credits: 30,
    },
    {
      name: 'Hailuo AI',
      desc: 'Новый игрок на рынке',
      isNew: true,
      credits: 40,
    },
    {
      name: 'Vidu AI',
      desc: 'Кинотеатральное качество',
      isNew: true,
      credits: 50,
    },
  ...
  return (
    <ErrorBoundary>
      <div className='flex flex-col h-[calc(100vh-var(--header-height,64px))] mesh-background'>
        ...
        <div className='flex-1 overflow-y-auto w-full relative z-[1]'>
                ...
                {capsuleOpen && (
                  ...
                  >
                    {videoProviders.map((p) => (
                      <div key={p.id}>
                        <div className='px-3 py-1.5 text-[11px] font-mono uppercase tracking-wider text-muted-foreground'>
                          {p.name}
                        </div>
                        {p.subModels.map((s) => (
                          <button
                            ...
                            className={cn(
                              ...
                              selectedSubModelId === s.id
                                ? 'bg-[rgba(232,84,32,0.12)]'
                                : 'hover:bg-secondary',
                            )}
                          >
          ...
          {!hasGenerations ? (
            <WelcomeBlock
              ...
              onScenarioClick={(scenario) => {
                ...
                if (scenario.providerId) {
                  const prov = videoProviders.find(
                    (p) => p.id === scenario.providerId,
                  );
                  if (prov) {
                    setSelectedProviderId(prov.id);
                    if (scenario.subModelId) {
                      const sub = prov.subModels.find(
                        (s) => s.id === scenario.subModelId,
                      );
            ...
            <>
              ...
              {isGenerating && (
                <GenerationLoader type='video' model={subModel?.name} />
              )}
              <div ref={feedEndRef} />
            </>
          )}
        ...
        <div
          ref={inputAreaRef}
          className='shrink-0 px-4 lg:px-6 pb-4 pt-1.5 bg-[var(--bg-primary)] relative z-[1]'
        >
          <div className='max-w-[780px] mx-auto'>
            <WorkspaceTabs variant='attached' />
            <div
              className={
                isGenerating ? 'glow-border-active' : 'glow-border-idle'
              }
            >
              <div className='rounded-[22px] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4 w-full transition-all duration-200 has-[textarea:focus]:border-[hsl(var(--primary))] has-[textarea:focus]:shadow-[0_0_0_3px_rgba(232,84,32,0.12),0_1px_4px_rgba(0,0,0,0.2)]'>
                ...
                <div className='flex items-center gap-2 flex-wrap'>
                  ...
                  {provider?.qualityOptions &&
                    provider.qualityOptions.length > 0 && (
                      <InlinePillDropdown
                        value={quality}
